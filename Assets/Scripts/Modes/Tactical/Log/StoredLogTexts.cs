...
static class StoredLogTexts
{
    ...
    /// <returns>
    /// True if the prey in the interaction is dead.<br></br>
    /// False if the prey in the interaction is alive.
    /// </returns>
    public static bool PreyDead(EventLog s)
    {
        return s.Prey.IsDead;
    }

    /// <returns>
    /// True if the prey in the interaction is dead in the pred's ball(s).<br></br>
    /// False if the prey in the interaction is alive <b>or</b> elsewhere in the pred.
    /// </returns>
    public static bool PreyCumgested(EventLog s)
    {
        return s.Prey.IsDead && s.preyLocation == PreyLocation.balls;
    }

    /// <returns>
    /// True if burping is allowed via the <u>Burp %</u> content setting being set higher than 0.<br></br>
    /// False if burping is disallowed via the <u>Burp %</u> content setting being set to 0.
    /// </returns>
    public static bool CanBurp()
    {
        return Config.BurpFraction > 0f;
    }

    /// <returns>
    /// True if farting is allowed via the <u>Fart on absorb</u> content setting.<br></br>
    /// False if farting is disallowed via the <u>Fart on absorb</u> content setting.
    /// </returns>
    public static bool Farts()
    {
        return Config.FartOnAbsorb;
    }

    /// <returns>
    /// True if scat content is allowed via the <u>Scat</u> content setting.<br></br>
    /// False if scat content is disallowed via the <u>Scat</u> content setting.
    /// </returns>
    public static bool ScatEnabled()
    {
        return Config.Scat;
    }

    /// <returns>
    /// True if scat content is allowed via the <u>Scat</u> content setting <b>and</b> the prey unit is in a stomach.<br></br>
    /// False if scat content is disallowed via the <u>Scat</u> content setting <b>or</b> the prey unit is not in a stomach.
    /// </returns>
    public static bool ScatPossible(EventLog s)
    {
        return Config.Scat && (s.preyLocation == PreyLocation.stomach || s.preyLocation == PreyLocation.stomach2);
    }

    /// <returns>
    /// True if lewd content is allowed via the <u>Lewd combat log text</u> content setting.<br></br>
    /// False if lewd content is disallowed via the <u>Lewd combat log text</u> content setting.
    /// </returns>
    public static bool Lewd()
    {
        return Config.LewdDialog;
    }

    /// <returns>
    /// True if hard vore content is allowed via the <u>Hard vore text enbled</u> content setting.<br></br>
    /// False if hard vore content is disallowed via the <u>Hard vore text enabled</u> content setting.
    /// </returns>
    public static bool HardVore()
    {
        return Config.HardVoreDialog;
    }

    /// <returns>
    /// True if the prey in the interaction is in the pred's stomach.<br></br>
    /// False if the prey in the interaction is elsewhere in the pred.
    /// </returns>
    public static bool InStomach(EventLog s)
    {
        return s.preyLocation == PreyLocation.stomach || s.preyLocation == PreyLocation.stomach2;
    }

    /// <returns>
    /// True if the prey in the interaction is in the pred's womb.<br></br>
    /// False if the prey in the interaction is elsewhere in the pred.
    /// </returns>
    public static bool InWomb(EventLog s)
    {
        return s.preyLocation == PreyLocation.womb;
    }

    /// <returns>
    /// True if the prey in the interaction is in the pred's stomach <b>or</b> womb.<br></br>
    /// False if the prey in the interaction is elsewhere in the pred.
    /// </returns>
    public static bool InStomachOrWomb(EventLog s)
    {
        return s.preyLocation == PreyLocation.stomach || s.preyLocation == PreyLocation.stomach2 || s.preyLocation == PreyLocation.womb;
    }

    /// <returns>
    /// True if the prey in the interaction is in the pred's breasts.<br></br>
    /// False if the prey in the interaction is elsewhere in the pred.
    /// </returns>
    public static bool InBreasts(EventLog s)
    {
        return s.preyLocation == PreyLocation.breasts || s.preyLocation == PreyLocation.leftBreast || s.preyLocation == PreyLocation.rightBreast;
    }
    ...
    /// False if the prey in the interaction is elsewhere in the pred.
    /// </returns>
    public static bool InBalls(EventLog s)
    {
        return s.preyLocation == PreyLocation.balls;
    }

    /// <summary>
    /// Checks a few things about the unit initianting an interaction to determine if the interaction can be considered their "first time" voring.
    /// </summary>
    /// <returns>
    /// False if the unit has previously digested another unit, is above level 10, is a Merencary, or the current game is a Pure Tactical battle.<br></br>
    /// True if none of the above conditions are met.<br></br>
    /// </returns>
    public static bool FirstTime(EventLog s)
    {
        return s.Unit.DigestedUnits == 0 && s.Unit.Level < 10 && s.Unit.Type != UnitType.Mercenary && s.Unit.Type != UnitType.SpecialMercenary && State.GameManager.PureTactical == false;
    }

    /// <summary>
    /// Checks a few things about the unit initianting an interaction to determine if the interaction can be considered their "first time" absorbing.<br></br>
    /// TODO: Find some way to check if the unit has previously absorbed another unit without digesting them (e.g. by being transferred a unit.)
    /// </summary>
    /// <returns>
    /// False if the unit has previously digested more than 1 unit, is above level 10, is a Merencary, or the current game is a Pure Tactical battle.<br></br>
    /// True if none of the above conditions are met.<br></br>
    /// </returns>
    public static bool FirstTimeAbsorption(EventLog s)
    {
        return s.Unit.DigestedUnits == 1 && s.Unit.Level < 10 && s.Unit.Type != UnitType.Mercenary && s.Unit.Type != UnitType.SpecialMercenary && State.GameManager.PureTactical == false;
    }

    /// <summary>
    /// Checks a few things about the target of an interaction to determine if the interaction can be considered their "first time" voring.<br></br>
    /// TODO: Find some way to check if the unit has previously absorbed another unit without digesting them (e.g. by being transferred a unit.)
    /// </summary>
    /// <returns>
    /// False if the unit has previously digested more than 1 unit, is above level 10, is a Merencary, or the current game is a Pure Tactical battle.<br></br>
    /// True if none of the above conditions are met.<br></br>
    /// </returns>
    public static bool TargetFirstTime(EventLog s)
    {
        return s.Target.DigestedUnits == 0 && s.Target.Level < 10 && s.Target.Type != UnitType.Mercenary && s.Target.Type != UnitType.SpecialMercenary && State.GameManager.PureTactical == false;
    }

    /// <returns>
    /// True if the unit and target of an interaction are on the same side.<br></br>
    /// False if the unit and target of an interaction are on different sides or the user performs the action on themself.
    /// </returns>
    public static bool Friendly(EventLog s)
    {
        return s.Unit.Side == s.Target.Side && s.Unit != s.Target;
    }

    /// <returns>
    /// True if the unit initiating the interaction has the <u>Endosoma</u> trait.<br></br>
    /// False if the unit initiating the interaction lacks the <u>Endosoma</u> trait.
    /// </returns>
    public static bool Endo(EventLog s)
    {
        return s.Unit.HasTrait(Traits.Endosoma);
    }

    /// <returns>
    /// True if the unit initiating the interaction has both the <u>Endosoma</u> and <u>HealingBelly</u> traits.<br></br>
    /// False if the unit initiating the interaction lacks either the <u>Endosoma</u> or <u>HealingBelly</u> traits.
    /// </returns>
    public static bool HealingEndo(EventLog s)
    {
        return s.Unit.HasTrait(Traits.Endosoma) && s.Unit.HasTrait(Traits.HealingBelly);
    }

    /// <returns>
    /// True if the pred and prey of an interaction are on the same side.<br></br>
    /// False if the pred and prey of an interaction are on different sides.
    /// </returns>
    public static bool FriendlyPrey(EventLog s)
    {
        return s.Unit.Side == s.Prey.Side;
    }

    /// <returns>
    /// True if the unit initiating the interaction is a human or humanoid/anthro.<br></br>
    /// False if the unit initiating the interaction is a monster.
    /// </returns>
    public static bool ActorHumanoid(EventLog s)
    {
        return s.Unit.Race < Race.Vagrants || s.Unit.Race >= Race.Selicia;
    }

    /// <returns>
    /// True if the target of the interaction has the <u>TheGreatEscape</u> trait.<br></br>
    /// False if the target of the interaction lacks the <u>TheGreatEscape</u> trait.
    /// </returns>
    public static bool HasGreatEscape(EventLog s)
    {
        return s.Target.HasTrait(Traits.TheGreatEscape);
    }

    /// <returns>
    /// True if the target of the interaction is afflicted with the <u>Prey's Curse</u> status effect.<br></br>
    /// False if the target of the interaction is <b>not</b> afflicted with the <u>Prey's Curse</u> status effect.
    /// </returns>
    public static bool Cursed(EventLog s)
    {
        return s.Target.GetStatusEffect(StatusEffectType.WillingPrey) != null;
    }
    ...
    /// False if the target of the interaction is <b>not</b> afflicted with the <u>Prey's Curse</u> status effect.
    /// </returns>
    public static bool Shrunk(EventLog s)
    {
        return s.Target.GetStatusEffect(StatusEffectType.Diminished) != null;
    }

    ///<param name="ratio">A <c>float</c> dictating how much larger the unit must be than the target to return <c>true</c>.</param>
    /// <returns>
    /// True if the unit initiating the interaction is at least <c>ratio</c> times as large as the target.<br></br>
    /// False if the unit initiating the interaction is smaller than that.
    /// </returns>
    public static bool SizeDiff(EventLog s, float ratio)
    {
        return State.RaceSettings.GetBodySize(s.Unit.Race) * s.Unit.GetScale(1) >= State.RaceSettings.GetBodySize(s.Target.Race) * s.Target.GetScale(1) * ratio;
    }

    ///<param name="ratio">A <c>float</c> dictating how much larger the unit must be than the prey to return <c>true</c>.</param>
    /// <returns>
    /// True if the unit initiating the interaction is at least <c>ratio</c> times as large as the prey.<br></br>
    /// False if the unit initiating the interaction is smaller than that.
    /// </returns>
    public static bool SizeDiffPrey(EventLog s, float ratio)
    {
        return State.RaceSettings.GetBodySize(s.Unit.Race) * s.Unit.GetScale(1) >= State.RaceSettings.GetBodySize(s.Prey.Race) * s.Target.GetScale(1) * ratio;
    }

    /// <returns>
    /// True if the unit initiating the interaction is on the same side as the prey and the target is digesting the prey.<br></br>
    /// False if the target is on the same side as the prey and has the <u>Endosoma</u> trait.
    /// </returns>
    ...
    internal static void InitializeLists()
    {
        ...
        bool ReqOSWLewd(EventLog s) => AttractedWarrior(s.Unit) != null && Lewd();
        ...
        bool ReqTargetCompatibleLewd(EventLog s) => RomanticTarget(s.Unit, s.Target) && Lewd();
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            actorRace: Race.Crux, priority: 9, conditional: s => CanBurp()),
            ...
            actorRace: Race.Panthers, priority: 9, conditional: s => HardVore() && InStomach(s)),
            ...
            actorRace: Race.Selicia, priority: 10, conditional: (i) => Lewd() && i.Target.HasBreasts && (i.Target.HasDick == false || Config.HermsCanUB)) ,
            ...
            new EventString((i) => $"As <b>{i.Unit.Name}</b> approaches <b>{i.Target.Name}</b>, the {GetRaceDescSingl(i.Target)}, seeing no mouth or other openings, thinks {GPPHimself(i.Target)} safe. Suddenly, <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> head opens up, and as <b>{i.Target.Name}</b> is rapidly swallowed down, {GPPHe(i.Target)} take{SIfSingular(i.Target)} a moment to think on what went wrong.",
            actorRace: Race.Aabayx, priority: 9),
            new EventString((i) => $"As <b>{i.Target.Name}</b> is swallowed down by a fellow Aabayx, all present are forced to marvel at the geometric impossibility on display before them, as <b>{i.Unit.Name}</b> somehow fits <b>{i.Target.Name}</b>'s icosahedral head through {GPPHis(i.Unit)} own icosahedral head.",
            ...
            priority: 8, conditional: s => CanBurp()),
            ...
            priority: 8, conditional: s => ScatPossible(s) && Lewd()),
            ...
            targetRace: Race.Cake, priority: 10, conditional: s => CanBurp() && HardVore()),
            ...
            targetRace: Race.Erin, priority: 26, conditional: (s) => Lewd() && HasGreatEscape(s)),
            ...
            targetRace: Race.Erin, priority: 26, conditional: s => Cursed(s) && Lewd() && HasGreatEscape(s)),

            //Abakhanskya Pred
            new EventString((i) => $"Tossing <b>{i.Target.Name}</b> up into the air, <b>{i.Unit.Name}</b> catches {GPPHim(i.Target)} by yawning her maw wide open. The poor {GetRaceDescSingl(i.Target)} wails in dismay as {GPPHe(i.Target)} plummets head-first into <b>Aba's</b> gullet, bulging out her fat neck briefly before fading into being simple bloat in a building-sized belly.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> screams as <b>{i.Unit.Name}</b> easily tucks {GPPHim(i.Target)} in her jaws and gulps them down into her waiting belly.",
            actorRace: Race.Abakhanskya, priority: 9),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> looks down at her gut curiously, wondering just where <b>{i.Target.Name}</b> went as {GPPHe(i.Target)} didn't appear to add any bulge to <b>Aba's</b> massive frame at all.  With a shrug, the dragoness turns back to the battle at hand.",
            conditional: s => SizeDiff(s, 5), actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> wails for help before being stuffed down by <b>{i.Unit.Name}</b>.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"Stuffed in head first, <b>{i.Target.Name}</b>'s ass is framed nicely by <b>{i.Unit.Name}</b>'s jaws before disappearing from the outside world.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Unit.Name}</b> swallows down <b>{i.Target.Name}</b> in one terrifyingly efficient gulp. With technique like that, this dragoness was clearly a hyper-capable predator...",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> flails about desperately, trying to get a grip on something, anything at all that might spare {GPPHim(i.Target)} {GPPHis(i.Target)} fate.  <b>{i.Unit.Name}</b> however, uncaring to her prey's plight, simply pushes {GPPHim(i.Target)} down into her gullet and swallows {GPPHim(i.Target)} with a loud *GLRK*.",
            actorRace: Race.Abakhanskya, priority: 9),
            ...
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Unit.Name}</b> toys with <b>{i.Target.Name}</b> before ultimately stuffing {GPPHim(i.Target)} down her gullet, finishing with a content sigh and a caress of her new belly bulge.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"Locked in <b>{i.Unit.Name}</b>'s iron grip, <b>{i.Target.Name}</b> begs for mercy.  She pretends to consider it for a moment, then shrugs and opens her massive jaws wide.  <b>{i.Target.Name}</b> has their scream squelched and replaced by the loud noise of Aba swallowing them down.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> is now adding extra inches to the swell of <b>{i.Unit.Name}</b>'s gut. Not <i>too</i> many inches since {GPPHe(i.Target)}'s so much smaller than the mighty khatunlao, but hey.  Its the little things that matter!",
            conditional: s => SizeDiff(s, 5), actorRace: Race.Abakhanskya, priority: 9),
            ...
            conditional: s => SizeDiff(s, 1), actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> is fearing that this may be it. This could very well be the last thing {GPPHe(i.Target)} ever sees. Locked inside of <b>{i.Unit.Name}</b>'s gut with the powerful acids already eating away at {GPPHim(i.Target)}...",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b>'s cries for aid can only barely be heard when going down <b>{i.Unit.Name}</b>'s throat. Then not at all when {GPPHe(i.Target)} makes that belly all the more round.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"The last face <b>{i.Target.Name}</b> makes before being snatched and snacked on by <b>{i.Unit.Name}</b> is one of utter despair.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b>'s eyes widen as {GPPHe(i.Target)} enters <b>{i.Unit.Name}</b>'s gut. The sea of chyme is already claiming {GPPHim(i.Target)}.",
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"Now inside of <b>{i.Unit.Name}</b>'s hungry stomach, <b>{i.Target.Name}</b> struggles to keep {GPPHis(i.Target)} head above the tides of digestive doom.",
            actorRace: Race.Abakhanskya, priority: 9),
        ...
        {
            new EventString((i) => $"<b>{i.Target.Name}</b> grows weaker as <b>{i.Unit.Name}</b>'s {i.preyLocation.ToSyn()} melt {GPPHim(i.Target)} away.",
            priority: 50, conditional: s => s.Unit.HasTrait(Traits.Annihilation)),
            ...
            priority: 8, conditional: s => ScatPossible(s)),
            ...
            priority: 8, conditional: s => HardVore() && InStomach(s)),
            ...
            actorRace: Race.Dogs, targetRace: Race.Cats, conditional: InStomach, priority: 8),
            ...
            new EventString((i) => $"The unique viral digestion of the Aabayx is taking a toll on <b>{i.Target.Name}</b>, as viruses progressively infect more of {GPPHis(i.Target)} body.",
            priority: 9, actorRace: Race.Aabayx),
            new EventString((i) => $"The chemical signals used by <b>{i.Unit.Name}</b>'s stomach are tricking the viral matrix that forms <b>{i.Target.Name}</b>'s body to willingly disassemble.",
            priority: 10, actorRace: Race.Aabayx, targetRace: Race.Aabayx),
            ...
            priority: 9, conditional: (s) => FirstTime(s) && HardVore()),
            ...
            priority: 9, conditional: s => Cursed(s) && InStomach(s) && Lewd() && ActorHumanoid(s)),
            ...
            priority: 9, conditional: s => Cursed(s) && InStomach(s) && Lewd()),
            ...
            new EventString((i) => $"As <b>{i.Target.Name}</b> blissfully sloshes back and forth in <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {i.preyLocation.ToSyn()}, {GPPHe(i.Target)} begin{SIfSingular(i.Target)} to rub {GPPHis(i.Target)} own belly, contemplating those that {GPPHe(i.Target)} {HasHave(i.Target)} also eaten. Their screams always made digestion seem terrifying, but now that {GPPHeIsAbbr(i.Target)} making a bulge on <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> frame, the experience is better than anything {GPPHe(i.Target)} could have imagined.",
            ...
            priority: 11, conditional: s => Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> hears <b>{i.Target.Name}</b> moan softly while {GPPHis(i.Unit)} {PreyLocStrings.SynAndPluralForm(i.preyLocation, "work")}  on {GPPHim(i.Target)}.",
            priority: 11, conditional: s => Friendly(s) && HealingEndo(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> hates the feeling of <b>{i.Target.Name}</b> inside {GPPHim(i.Unit)}, but keeps {GPPHim(i.Target)} inside for {GPPHis(i.Target)} own good.",
            priority: 11, conditional: s => Friendly(s) && HealingEndo(s)),
            ...
            priority: 11, conditional: s => Friendly(s) && Endo(s) && InStomach(s) && Lewd() && ActorHumanoid(s)),
            ...
            priority: 8, conditional: (s) => Friendly(s) && HardVore()),
            ...
            priority: 8),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.SynAndPluralForm(i.preyLocation, "quiver", "and shake")}, as <b>{i.Target.Name}</b> tries to make {GPPHim(i.Unit)} {i.preyLocation.ToVerb()} {GPPHim(i.Target)} out.",
            priority: 8),
            ...
            //new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.SynAndPluralForm(i.preyLocation, "rumble")} as it greets fresh prey, eager for more.",
            //priority: 8),
            ...
            priority: 8, conditional: InStomach),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.SynAndPluralForm(i.preyLocation, "wobble")} vigorously as <b>{i.Target.Name}</b> tries to escape.",
            priority: 8),
            ...
            priority: 8),
            new EventString((i) => $"<b>{i.Target.Name}</b> might not see the light of day again if <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {PreyLocStrings.SynAndPluralForm(i.preyLocation, "continue")} to be left in charge of {GPPHim(i.Target)}.",
            priority: 8),
            ...
            priority: 8, conditional: (s) => InStomach(s) && CanBurp()),
            ...
            priority: 8, conditional: (s) => InStomach(s) && CanBurp()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> moans softly while {GPPHis(i.Unit)} stuffed {PreyLocStrings.SynAndPluralForm(i.preyLocation, "rumble")} ominously.",
            priority: 8),
            ...
            priority: 8, conditional: s => HardVore()),
            ...
            priority: 8, conditional: s => TacticalUtilities.Units.Where(a => a.Unit.Side == s.Target.Side && a.Unit != s.Target).Count() >= 1),
            new EventString((i) => $"<b>{i.Target.Name}</b> desperately struggles inside <b>{i.Unit.Name}</b>, knowing that nobody is coming to save {GPPHim(i.Target)}.",
            priority: 8, conditional: s => TacticalUtilities.Units.Where(a => a.Unit.Side == s.Target.Side && a.Unit != s.Target).Count() >= 1),
            ...
            new EventString((i) => $"<b>{i.Target.Name}</b> promises that {GPPHe(i.Target)} will eat <b>{i.Unit.Name}</b> once {GPPHe(i.Target)} escapes. <b>{i.Unit.Name}</b> laughs and pats {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}.",
            priority: 8),
            ...
            priority: 8),
            new EventString((i) => $"<b>{i.Unit.Name}</b> tells <b>{i.Target.Name}</b> to be thankful as {GPPHis(i.Unit)} {PreyLocStrings.PartIsAre(i.preyLocation)} far more gentle than {GPPHis(i.Unit)} friends'.",
            priority: 8),
            ...
            priority: 8, conditional: (s) => ReqOSWBelly(s) && Lewd()),
            ...
            priority: 8, conditional: s => ReqOSWBelly(s) && AttractedWarrior(s.Unit).HasDick && Lewd()),
            ...
            priority: 8, conditional: s =>ReqOSWLewd(s) && HardVore()),
            ...
            priority: 8, conditional: s => ReqOSWBelly(s) && Lewd() && ActorHumanoid(s)),
            ...
            priority: 8, conditional: s => Lewd() && HardVore() ),
            ...
            priority: 8, conditional: s => Lewd() && InStomachOrWomb(s)),
            ...
            priority: 8, conditional: s => Lewd() && InStomach(s)),
            ...
            priority: 8, conditional: s => InStomach(s) && Farts() && ActorTail(s)),
            ...
            priority: 8, conditional: s => InStomach(s) && ScatPossible(s) && ActorHumanoid(s)),
            ...
            priority: 8, conditional: s => InStomach(s) && ActorHumanoid(s)),
            new EventString((i) => $"\"O-Oh my!\" <b>{i.Unit.Name}</b> exclaims as {GPPHis(i.Unit)} sagging belly begins to sway back and forth with muffled lust filled moans as <b>{i.Target.Name}</b> pleasures {GPPHimself(i.Target)}. \"You’re a naughty one, aren’t you!\"",
            priority: 8, conditional: s => InStomach(s) && Lewd() && ActorHumanoid(s)),
            new EventString((i) => $"\"You know,\" <b>{i.Unit.Name}</b> idly muses while stroking {GPPHis(i.Unit)} jiggling belly, \"It’s hard to feel lonely when you’ve got someone so close by.\"",
            priority: 8, conditional: s => InStomach(s) && ActorHumanoid(s)),
            ...
            priority: 9, conditional: s=> Cursed(s) && Lewd()),
        ...
        // Belly rub messages priority guide:
        // If a line involves various factors, give it the priority of the highest-prioritized factor in this list.
        // If you're adding more lines, keep this in mind: the game will look for all lines for which the conditions are met,
        // then find the highest priorty in that list and randomly use a matching line with that priority.
        // priority  5: detailed race-specific (i.e. pred race has important difference in behavior when digesting)
        // priority  4: endo; healing endo
        // priority  3: enemy; self
        // priority  2: simple race-specific, worship/idolization
        // priority  1: baseline; generic
        BellyRubMessages = new List<EventString>()
        {
            // Pred is FeralLion and is self-rubbing
            ...
            priority: 5, actorRace: Race.FeralLions, conditional: s => s.Target == s.Unit),
            ...
            priority: 5, actorRace: Race.FeralLions, conditional: s => s.Target == s.Unit && !PreyDead(s)),
            ...
            priority: 5, actorRace: Race.FeralLions, conditional: s => s.Target == s.Unit && !PreyDead(s) && CanBurp()),
            ...
            priority: 5, actorRace: Race.FeralLions, conditional: s => s.Target == s.Unit && InStomach(s) && !InWomb(s) && PreyDead(s) && Farts() && PreyDead(s)),
            ...
            // Pred and Belly Rubber are both FeralLions
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => PreyDead(s)),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => !PreyDead(s)), // TODO: Cattitude or volunteers, implement a way to tell what appendage a creature uses for interactions. I think an enumerable would be a good fit, but can be any sort of presence check too.
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit),
            new EventString((i) => $"<b>{i.Unit.Name}</b> finds <b>{i.Target.Name}</b> lazing on {GPPHis(i.Target)} back and gives {GPPHis(i.Target)} {PreyLocStrings.ToSyn(i.preyLocation)} a vigorous nuzzle, pressing {GPPHis(i.Unit)} face into the tummy{((PreyDead(i) && Farts() && InStomach(i)) ? ", stirring the liquefied prey and causing a little bit of <b>" + i.Prey.Name + "</b> gas to be squished out of "+ GPPHis(i.Target) + " butt": "")}.",
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit && TacticalUtilities.GetActorOf(s.Unit).HasBelly),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit && InStomach(s)),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit && InWomb(s)),
            ...
            priority: 5, actorRace: Race.FeralLions, targetRace: Race.FeralLions, conditional: s => s.Target != s.Unit && SizeDiff(s,1f)),
            ...
            priority: 5, conditional: s => s.Target == s.Unit && (s.Target.Race == Race.Serpents || s.Target.Race == Race.Lamia)),
            ...
            priority: 3, conditional: s => s.Target == s.Unit),
            ...
            priority: 5, conditional: s => TargetFirstTime(s) && Friendly(s)),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && s.Target.HasBreasts),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s)),
            ...
            priority: 1, conditional: s => ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            ...
            priority: 1, conditional: s => ReqTargetCompatibleLewd(s)),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && s.Target.HasDick),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatible(s) && InStomach(s)),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatible(s)),
            ...
            priority: 1, conditional: s => ReqTargetCompatible(s)),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && InStomach(s)),
            ...
            priority: 1, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            ...
            priority: 1, conditional: s => ReqTargetCompatible(s) && ActorHumanoid(s) && Friendly(s)),
            ...
            priority: 1, conditional: s => Friendly(s)),
            ...
            targetRace: Race.Deer, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && FriendlyPrey(s) && !Endo(s)),
            ...
            targetRace: Race.Deer, priority: 2, conditional: s => ReqTargetCompatibleLewd(s)),
            ...
            targetRace: Race.Deer, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && s.Unit.GetGender() == Gender.Male),
            ...
            targetRace: Race.Deer, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && !FirstTime(s)),
            ...
            targetRace: Race.Cats, priority: 9),
            new EventString((i) => $"The attentions on {GPPHis(i.Target)} bursting gut from the one {GPPHe(i.Target)} love{SIfSingular(i.Target)} cause <b>{i.Target.Name}</b> to inadvertently start purring. The feline predator catches {GPPHimself(i.Target)} and stops but it’s too late, <b>{i.Unit.Name}</b> already thinks {GPPHeIs(i.Target)} adorable.",
            targetRace: Race.Cats, priority: 9, conditional: s => Friendly(s) && ReqTargetCompatible(s)),
            ...
            targetRace: Race.Cats, priority: 9, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && s.Target.GetGender() == Gender.Female),
            ...
            targetRace: Race.Cats, priority: 9, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s)),
            ...
            targetRace: Race.Cats, priority: 9, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && s.Target.GetGender() == Gender.Female),

            // Dog (& dog-adjacent) belly rubs
            new EventString((i) => $"\"Who's a good {GetGenderString(i.Target, "girl", "boy", (i.Target.Race == Race.Dogs ? "dog" : "wolf"))}?\" <b>{i.Unit.Name}</b> says as {GPPHe(i.Unit)} rub{SIfSingular(i.Unit)} <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly. The {(i.Target.Race == Race.Dogs ? "dog" : "wolf")} lies down, {GPPHis(i.Target)} tongue lolling out, tail rapidly wagging, and leg kicking as <b>{i.Unit.Name}</b> rubs {GPPHis(i.Target)} belly in all the right places.{GetRandomStringFrom("", (i.Target.Race == Race.Dogs ? $" \"I am! I am a good {GetGenderString(i.Target, "boy", "girl", "dog")}!\" <b>{i.Target.Name}</b> happily exclaims." : (i.Target.Race == Race.Wolves ? $" \"I am!\" <b>{i.Target.Name}</b> exclaims." : $" \"You are! Yes you are!\" <b>{i.Unit.Name}</b> exclaims.")))}",
            priority: 2, conditional: s => s.Target != s.Unit && (s.Target.Race == Race.Wolves || s.Target.Race == Race.FeralWolves || s.Target.Race == Race.Dogs)),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ActorHumanoid(s)),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) || (s.Unit.HasTrait(Traits.SeductiveTouch) && TacticalUtilities.Units.Where(actor => actor.Unit == s.Target)?.FirstOrDefault().TurnsSinceLastParalysis <= 0) ),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatible(s)),
            ...
            targetRace: Race.Dogs, priority: 2),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s)),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && s.Target.GetGender() == Gender.Female),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && (s.Target.Race != s.Unit.Race || s.Target.Level < s.Unit.Level) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            new EventString((i) => $"“You know,” <b>{i.Target.Name}</b> says imperiously as {GPPHis(i.Target)} beloved presses into {GPPHis(i.Target)} soft, supple body, “I’m actually very naughty.” {GPPHe(i.Target)} rub{SIfSingular(i.Target)} the side of {GPPHis(i.Target)} bloated {(Config.HardVoreDialog ? "meat" : $"{GetRaceDescSingl(i.Prey)}")}-filled gut as it lets out a groan and leans against <b>{i.Unit.Name}</b> while whispering into {GPPHis(i.Unit)} ear, “You should probably put a collar on me. Then we can start to work on... discipline.”",
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && InStomach(s)),
            new EventString((i) => $"“You know I can smell it on you, right?” <b>{i.Target.Name}</b> states as <b>{i.Unit.Name}</b> lovingly tends to {GPPHis(i.Target)} {(Config.HardVoreDialog ? $"{i.preyLocation.ToSyn()}, full of digestive juices and melting flesh" : $"{GetRaceDescSingl(i.Prey)}-filled {i.preyLocation.ToSyn()}")} as it lets out a groan and leans against <b>{i.Unit.Name}</b> while whispering into {GPPHis(i.Unit)}. “Your lusting, your desire to have your way with me. The air’s thick with the scent,” the canine predator continues as lovingly tended gut rumbles, “Don’t worry. I want that too.”",
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && InStomach(s)),
            ...
            targetRace: Race.Dogs, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && InStomachOrWomb(s)),

            // Fox/Youko belly rubs (Figured out how to set a line for multiple races)
            new EventString((i) => $"<b>{i.Target.Name}</b>, with great melodrama, begins to wail about {GPPHis(i.Target)} pained {i.preyLocation.ToSyn()} as it bubbled away its last victim. <b>{i.Unit.Name}</b>, naïve against the wiley fox’s acting, immediately starts lathering the bushy tailed warrior with attention and rubbing.",
            priority: 2, conditional: s => (s.Target.Race == Race.Foxes || s.Target.Race == Race.Youko) && InStomach(s) && s.Prey.IsDead),
            new EventString((i) => $"\"You know,\" <b>{i.Target.Name}</b> muses with a sly grin as <b>{i.Unit.Name}</b> tends to {GPPHis(i.Target)} sloshing {i.preyLocation.ToSyn()}, \"You could always massage me from the inside, it’d be much more relaxing. I promise I wouldn’t let you melt away!\" The trickster is surprised and flattered when {GPPHis(i.Target)} attendant doesn’t oppose the idea because {GPPHe(i.Unit)} trust{SIfSingular(i.Unit)} {GPPHim(i.Target)} so much.",
            priority: 2, conditional: s => (s.Target.Race == Race.Foxes || s.Target.Race == Race.Youko) && ReqTargetCompatible(s) && ActorHumanoid(s) && InStomach(s)),
            ...
            targetRace: Race.Wolves, priority: 2, conditional: s => InStomach(s)),
            ...
            targetRace: Race.Wolves, priority: 2),
            new EventString((i) => $"Not wanting to share {GPPHis(i.Target)} new mate with {GPPHis(i.Target)} packmates, <b>{i.Target.Name}</b> suddenly starts to aggressively rub {GPPHis(i.Target)} gorged stomach against <b>{i.Unit.Name}</b> as {GPPHe(i.Unit)} pass{EsIfSingular(i.Unit)} by, marking {GPPHim(i.Unit)} with {GPPHis(i.Target)} scent.",
            targetRace: Race.Wolves, priority: 2, conditional: s => Friendly(s) && s.Target != s.Unit && ReqTargetCompatible(s) && InStomach(s)),
            ...
            targetRace: Race.Wolves, priority: 2, conditional: s => ActorHumanoid(s)),
            ...
            targetRace: Race.Wolves, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && s.Target.GetGender() == Gender.Female),
            ...
            targetRace: Race.Bunnies, priority: 2, conditional: s => Friendly(s)),
            ...
            targetRace: Race.Bunnies, priority: 2, conditional: s => HardVore() && InStomach(s)),
            ...
            targetRace: Race.Bunnies, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && InStomach(s)),
            ...
            targetRace: Race.Lizards, priority: 2, conditional: s => InStomach(s)),
            ...
            targetRace: Race.Lizards, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            ...
            targetRace: Race.Lizards, priority: 2, conditional: s => Friendly(s) && InStomach(s) && HardVore()),
            ...
            targetRace: Race.Lizards, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && InStomach(s)),
            new EventString((i) => $"\"<b>{i.Target.Name}</b> will never eat you, even if you become weak,\" the {GetRaceDescSingl(i.Target)} predator says while <b>{i.Unit.Name}</b> caresses {GPPHis(i.Target)} sloshing stomach. Before a thanks can be offered, the {GetRaceDescSingl(i.Target)} continues, \"Reward this warrior’s compassion with passionate mating.\"",
            targetRace: Race.Lizards, priority: 2, conditional: s => ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            ...
            targetRace: Race.Imps, priority: 2),
            ...
            targetRace: Race.Imps, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && InStomach(s)),
            ...
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && InStomach(s)),
            ...
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && s.Unit.HasBreasts),
            ...
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> sits down on {GPPHis(i.Target)} fat laden ass, letting {GPPHis(i.Target)} belly rest on the ground in an attempt to relieve {GPPHis(i.Target)} indigestion as {GPPHis(i.Target)} meal passes from one stomach to the next. <b>{i.Unit.Name}</b>, seeing {GPPHis(i.Target)} discomfort arrives and attends to {GPPHis(i.Target)} sloshing belly. <b>{i.Target.Name}</b> basks in the attention, leaning back on {GPPHis(i.Target)} elbows as {GPPHeIsAbbr(i.Target)} rubbed down.",
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && InStomach(s)),
            ...
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && s.Unit.HasBreasts && InStomach(s)),
            ...
            targetRace: Race.Taurus, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && InStomachOrWomb(s)),

            // Dragon belly rubs
            new EventString((i) => $"At <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> behest, <b>{i.Unit.Name}</b> begins to tend to the great dragon’s mighty belly as it melts {GPPHis(i.Target)} foes away {(Config.HardVoreDialog ? $"into a fine sludge" : "")}.",
            targetRace: Race.Dragon, priority: 2, conditional: s => InStomachOrWomb(s)),
            ...
            targetRace: Race.Dragon, priority: 2, conditional: s => Friendly(s)),
            ...
            targetRace: Race.Dragon, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s)),
            new EventString((i) => $"Servants like <b>{i.Unit.Name}</b> remind <b>{i.Target.Name}</b> why {GPPHe(i.Target)} do{EsIfSingular(i.Target)}n’t simply devour everyone in {GPPHis(i.Target)} path. The thought, however, conflicts with {GPPHis(i.Target)} sloshing midsection melting a {(GetActor(i.Unit).PredatorComponent.PreyInLocation(PreyLocation.stomach, true) > 1 ? $"number of mortals" : "mortal")}{(Config.HardVoreDialog ? $"' flesh and bone" : " away")} as the aforementioned servant rubs it.",
            targetRace: Race.Dragon, priority: 2, conditional: s => Friendly(s)),
            ...
            targetRace: Race.Dragon, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s)),
            ...
            targetRace: Race.Dragon, priority: 2, conditional: s => Friendly(s) && ReqTargetCompatibleLewd(s) && ActorHumanoid(s) && InStomach(s)),
            ...
            priority: 2, conditional: s => (s.Target.Race == Race.Dragon || s.Target.Race == Race.EasternDragon) && InStomach(s)),
            ...
            actorRace: Race.Kobolds, priority: 2, conditional: s => (s.Target.Race == Race.Dragon || s.Target.Race == Race.EasternDragon) && InStomachOrWomb(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> hops onto <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> belly and begins to knead and rub it, elicting a pleasured groan from {GPPHis(i.Unit)} draconic master.",
            actorRace: Race.Kobolds, priority: 2, conditional: s => (s.Target.Race == Race.Dragon || s.Target.Race == Race.EasternDragon) && InStomachOrWomb(s)),
            ...
            priority: 3, conditional: s => s.Target == s.Unit && Lewd()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> moans in pleasure as {GPPHe(i.Unit)} rub{SIfSingular(i.Unit)} {GPPHis(i.Unit)} belly, hoping to speed up <b>{i.Prey.Name}</b>'s recovery.",
            priority: 4, conditional: s => s.Target == s.Unit && HealingEndo(s) && FriendlyPrey(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> hums sweetly as {GPPHe(i.Unit)} massage{SIfSingular(i.Unit)} {GPPHis(i.Unit)} belly, lovingly kneading <b>{i.Prey.Name}</b>'s body.",
            priority: 4, conditional: s => s.Target == s.Unit && HealingEndo(s) && FriendlyPrey(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> massages {GPPHimself(i.Unit)}, shifting <b>{i.Prey.Name}</b> into a more comfortable position inside {GPPHim(i.Unit)}.",
            priority: 4, conditional: s => s.Target == s.Unit && Endo(s) && FriendlyPrey(s)),
            new EventString((i) => $"{i.Unit.Name} puts {GPPHis(i.Unit)} hands on {GPPHis(i.Unit)} belly, feeling <b>{i.Prey.Name}</b> within.",
            priority: 4, conditional: s => s.Target == s.Unit && Endo(s) && FriendlyPrey(s)),
            new EventString((i) => $"{i.Unit.Name} slaps {GPPHis(i.Unit)} belly, savoring the feeling of <b>{i.Prey.Name}</b> inside.",
            priority: 4, conditional: s => s.Target == s.Unit && Endo(s) && FriendlyPrey(s)),
            new EventString((i) => $"{i.Unit.Name} swings {GPPHis(i.Unit)} gut side to side, enjoying the feeling of <b>{i.Prey.Name}</b> sloshing around inside.",
            priority: 4, conditional: s => s.Target == s.Unit && Endo(s) && FriendlyPrey(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> gives <b>{i.Target.Name}</b> an amorous hug, squishing <b>{i.Prey.Name}</b> between the two of them.",
            priority: 4, conditional: s => Endo(s) && Friendly(s) && FriendlyPrey(s) && ReqTargetCompatible(s)),
            new EventString((i) => $"<b>{i.Prey.Name}</b> lets out a soft moan as <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> hands wrap around <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> engorged belly.",
            priority: 4, conditional: s => Endo(s) && Friendly(s) && FriendlyPrey(s) && ReqTargetCompatibleLewd(s)),
            ...
            // Worship/Idolization belly rubs
            ...
            priority: 2, conditional: s => s.Unit.Level + 9 < s.Target.Level),
            ...
            priority: 2, conditional: s => s.Unit.Level + 9 < s.Target.Level && Friendly(s)),
            ...
            priority: 2, conditional: s => s.Unit.Level + 9 < s.Target.Level && Lewd()),
            ...
            priority: 2, conditional: s => s.Unit.Level + 9 < s.Target.Level && Friendly(s) && Lewd()),
            ...
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side && (s.Unit.Level + 9 < s.Target.Level)),
            ...
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side && ReqTargetCompatible(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> dextrously closes in and starts kneading <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {i.preyLocation.ToSyn()} in the middle of battle, making sure {GPPHis(i.Unit)} struggling former ally is processed nicely within the {GetRandomStringFrom("predator", GetRaceDescSingl(i.Unit))}.",
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side),
            ...
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side),
            new EventString((i) => $"Instead of surrendering, <b>{i.Unit.Name}</b> seems to be offering a massage. A really good massage. <b>{i.Target.Name}</b> completely has {GPPHis(i.Target)} guard down as {GPPHe(i.Target)} get{SIfSingular(i.Prey)} lost in the bliss of feeling {GPPHis(i.Target)} prey melt away in record time, but <b>{i.Unit.Name}</b> doesn't take advantage, tending to the {GetRaceDescSingl(i.Unit)} dutifully.",
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side),
            new EventString((i) => $"<b>{i.Target.Name}</b> doesn't regret letting an unarmed <b>{i.Unit.Name}</b> touch {GPPHis(i.Target)} noisy belly. The {GetRaceDescSingl(i.Unit)} expertly shifts and softens the {GetRaceDescSingl(i.Target)} food that {GPPHe(i.Unit)} used to call an ally. Perhaps this is a sign {GPPHe(i.Unit)} wish{EsIfSingular(i.Unit)} to be brought inside too!",
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side && ActorHumanoid(s)),
            ...
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side && !s.Prey.IsDead),
            ...
            priority: 3, conditional: s => s.Target.Side != s.Unit.Side && !s.Prey.IsDead && InStomach(s)),

            // Betrayal belly rubs
            new EventString((i) => $"<b>{i.Unit.Name}</b> caresses <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {i.preyLocation.ToSyn()} with such devotion that <b>{i.Prey.Name}</b> can't help but despair as {GPPHe(i.Prey)} understand{SIfSingular(i.Prey)} {GPPHis(i.Prey)} allies wrote {GPPHim(i.Prey)} off, only considering {GPPHim(i.Prey)} food now, which should just hurry up and liquefy. At least they strive to make it quick and painless?",
            priority: 3, conditional: s => !s.Prey.IsDead && Betrayal(s)),
        };
        ...
        BreastRubMessages = new List<EventString>()
        {
            ...
            new EventString((i) =>$"<b>{i.Unit.Name}</b> moans and hugs <b>{i.Target.Name}</b> tightly and grinds their breasts against one another, sending tingling bliss down both their spines.", actorRace: Race.Succubi, priority: 9, conditional: s=> Friendly(s) && ReqTargetCompatibleLewd(s) && ActorBoobs(s)),
        };
        ...
        TailRubMessages = new List<EventString>()
        {
            ...
            new EventString((i) =>$"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> tail exhales a puff of acrid gas from the steady massages <b>{i.Unit.Name}</b> is giving it. <b>{i.Target.Name}</b> soon finds {GPPHimself(i.Unit)} dunked into strong stomach acids.", priority: 8, conditional:s=> HardVore() && CanBurp()),

            // Can we do an OR for actorrace? 
            // We sure can :)
            new EventString((i) =>$"<b>{i.Unit.Name}</b> massages each lumpy bump on {(i.Unit == i.Target ? GPPHis(i.Target) : "<b>" + i.Target.Name + "</b>'s")} tail, enjoying each bulge slowly soften and flatten.", priority: 9, conditional: s => s.Unit.Race == Race.Vipers || s.Unit.Race == Race.Lamia),
        ...
        BallMassageMessages = new List<EventString>()
        {
            ...
            actorRace: Race.Kobolds, priority: 10, conditional: s => Lewd() && s.Target.Race == Race.Dragon || s.Target.Race == Race.EasternDragon),
            ...
            priority: 15, conditional: s => Lewd() && (s.Unit.Level + 9 < s.Target.Level)),
        ...
        CockVoreMessages = new List<EventString>()
        {
            ...
            priority: 8),
            new EventString((i) => $"<b>{i.Unit.Name}</b> lewdly sways {GPPHis(i.Unit)} hips, giving friend and foe alike the sight of <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> body disappearing into {GPPHis(i.Unit)} cock.",
            priority: 8),
            ...
            targetRace: Race.Slimes, priority: 9, conditional: s => Lewd()),
            ...
            priority: 11, conditional: s => Shrunk(s) && Lewd()),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> stuffs <b>{i.Target.Name}</b> into {GPPHis(i.Unit)} cock.",
            priority: 25, conditional: HasGreatEscape),
            new EventString((i) => $"In a humping frenzy accompanying <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> descent into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> gurgling balls', <b>{i.Unit.Name}</b> doesn't notice <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> smug expression. \"Enjoy it while it lasts\" - the prey says disappearing inside.",
            priority: 25, conditional: HasGreatEscape),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> shaft greedily consumes <b>{i.Target.Name}</b>, {GPPHis(i.Target)} form engorging the pred's balls. <b>{i.Target.Name}</b> settles within and starts probing the walls of {GPPHis(i.Target)} fleshy prison, looking for a way out.",
            priority: 25, conditional: HasGreatEscape),
        ...
        AnalVoreMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> knocks over <b>{i.Target.Name}</b> and plants {GPPHis(i.Unit)} rump right on {GPPHis(i.Target)} face to let loose a blast of vile farts, numbing {GPPHis(i.Target)} senses. <b>{i.Target.Name}</b> barely has a grip on what's happening as {GPPHe(i.Target)} vanish{EsIfSingular(i.Target)} up the gassy tailhole.", priority: 9, conditional: s => Farts() && SizeDiff(s, 3)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> plants {GPPHis(i.Unit)} rump on <b>{i.Target.Name}</b> and rubs {GPPHis(i.Unit)} filthy tailhole all over their comparatively tiny body. In the heat of the moment, the {GetRaceDescSingl(i.Unit)} lets lose all of {GPPHis(i.Unit)} flatulence upon <b>{i.Target.Name}</b>. When {GPPHe(i.Unit)} get{SIfSingular(i.Unit)} back up, {GPPHis(i.Unit)} thinks the little {GetRaceDescSingl(i.Target)} got vaporized by {GPPHis(i.Unit)} farts, until {GPPHe(i.Unit)} feel{SIfSingular(i.Unit)} a squirm inside {GPPHis(i.Unit)} ass.", priority: 9, conditional: s => Farts() && ScatPossible(s) && SizeDiff(s, 3)),
            ...
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> squirms and thrashes about, trying to free {GPPHimself(i.Target)} from the grasp of {GPPHis(i.Target)} vicious predator... and then {( Lewd() ? "moans in delight" : "giggles")} as the last of {GPPHim(i.Target)} slides past the sphincter.\n<b>{i.Unit.Name}</b> appreciates the act and caresses {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}.",
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
        ...
        UnbirthMessages = new List<EventString>()
        {
            ...
            new EventString((i) =>$"In the heat of {GPPHis(i.Unit)} needs, <b>{i.Unit.Name}</b> desides to use <b>{i.Target.Name}</b> as a living dildo – Unfortunately, {GPPHis(i.Unit)} toys have a tendency to disappear.", priority: 8, conditional: s => Lewd()),
            ...
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> squirms and thrashes about, trying to free {GPPHimself(i.Target)} from the grasp of {GPPHis(i.Target)} vicious predator... and then {( Lewd() ? "moans in delight" : "giggles")} as the last of {GPPHim(i.Target)} slide{SIfSingular(i.Target)} past the nether lips.\n<b>{i.Unit.Name}</b> appreciates the act and caresses {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}.",
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
        ...
        TailVoreMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> flashes {GPPHis(i.Unit)} {(i.Unit.HasDick ? "dick" : "breasts")} at <b>{i.Target.Name}</b>, distracting {GPPHim(i.Target)} long enough to swallow the {GetPreyDesc(i.Target)} {GetRaceDescSingl(i.Target)} feet first with {GPPHis(i.Unit)} tail.", actorRace: Race.Succubi, priority: 9, conditional: s => Lewd() && TargetHumanoid(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> seizes <b>{i.Target.Name}</b> in a tight hug, grinding {GPPHis(i.Unit)} {(i.Unit.HasDick ? "dick" : "breasts")} against <b>{i.Target.Name}</b> before pulling <b>{i.Target.Name}</b> down into the awaiting tail maw.", actorRace: Race.Succubi, priority: 9, conditional: s => Lewd() && TargetHumanoid(s)),
            ...
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> squirms and thrashes about, trying to free {GPPHimself(i.Target)} from the grasp of {GPPHis(i.Target)} vicious predator... and then {( Lewd() ? "moans in delight" : "giggles")} as the last of {GPPHim(i.Target)} vanish{EsIfSingular(i.Target)} into the tail.\n<b>{i.Unit.Name}</b> appreciates the act and caresses {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}.",
            priority: 12, conditional: s => Friendly(s) && Endo(s)),
        ...
        DigestionDeathMessages = new List<EventString>()
        {
            ...
            priority: 9, conditional: s=> HardVore() && InStomach(s) && TargetHumanoid(s)),
            ...
            actorRace: Race.Panthers, priority: 9, conditional: s=> InStomach(s) && CanBurp()),
            ...
            priority: 10, conditional: s => s.Unit.HasTrait(Traits.InstantDigestion) && InStomach(s) && HardVore()),
            ...
            priority: 10, conditional: s => s.Unit.HasTrait(Traits.InstantDigestion) && InBalls(s) && HardVore()),
            ...
            priority: 9, conditional: s => Cursed(s) && Lewd() && InStomach(s) && ScatPossible(s)),
            ...
            priority: 9, conditional: s => Cursed(s) && Lewd() && InStomach(s) && Farts()),
            ...
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"<b>{i.Target.Name}</b> accepts {GPPHis(i.Unit)} destiny is to become fuel for {GPPHis(i.Unit)} better, Abakhanskya. {GPPHe(i.Unit)} slips beneath the digestive tide and wonders just where on the predator's body {GPPHe(i.Unit)}'ll end up.",
            actorRace: Race.Abakhanskya, priority: 9),
        ...
        AbsorptionMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"Having been robbed of all {GPPHis(i.Target)} strength, <b>{i.Target.Name}</b> is absorbed into <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {i.preyLocation.ToSyn()}.",
            priority: 100, conditional: s => s.Unit.HasTrait(Traits.Annihilation)),
            ...
            //Scat disposal
            new EventString((i) => $"<b>{i.Unit.Name}</b> enjoys feeling the weight of <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> remains squeeze out of {GPPHis(i.Unit)} ass.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> takes a lewd pleasure in <b>{i.Target.Name}</b> sliding out of {GPPHis(i.Unit)} rectum as several thick turds.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> smirks, noticing {GPPHis(i.Unit)} ass feels heavier after absorbing and eliminating <b>{i.Target.Name}</b>.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> takes a quick dump to purge <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> foul remains from {GPPHis(i.Unit)} guts.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> shits out a large, steamy batch of fresh manure. This wasn't the escape <b>{i.Target.Name}</b> had been looking for in {GPPHis(i.Unit)} dark stomach.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> moans in disgust and pleasure, feeling <b>{i.Target.Name}</b> squeeze through {GPPHis(i.Unit)} colon and out of {GPPHis(i.Unit)} plump {PreyLocStrings.ToSyn(PreyLocation.anal)}.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> presses down on {GPPHis(i.Unit)} {i.preyLocation.ToSyn()}, lolling out {GPPHis(i.Unit)} tongue in relief while letting loose one huge log of shit. There's <b>{i.Target.Name}</b>...", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Target.Name}</b> is completely digested, dumped out as a gross scat pile behind <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> paws.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Target.Name}</b> finishes digesting in <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> bowels, and is promptly eliminated from {GPPHis(i.Unit)} body.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> squats to deposit <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> used-up remains in a messy pile below {GPPHis(i.Unit)} tail.", priority: 9, conditional: s => ScatPossible(s) && ActorTail(s)),
            new EventString((i) => $"After squeezing out <b>{i.Target.Name}</b> as a nice, {GetRandomStringFrom("nasty", "creamy", "warm", "greasy")} {GetRandomStringFrom("log", "turd", "brown lump")}, <b>{i.Unit.Name}</b> winks at <b>{PotentialNextPrey(i.Unit).Name}</b>, as if to say \"You're next!\".\n...{GPPHis(i.Unit)} sphincter seems to do the same.", priority: 9, conditional: s=> ScatPossible(s) && (CanAddressPlayer(s) || PotentialNextPrey(s.Unit).Name != "You, the player" )),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b> looks back at the vile mass of <b>{i.Target.Name}</b> that just slid out {GPPHis(i.Unit)} behind, then at <b>{PotentialNextPrey(i.Unit).Name}</b>, then back at {GPPHis(i.Unit)} {GetRandomStringFrom("dump", "shit", "crap")}, before licking {GPPHis(i.Unit)} lips at <b>{PotentialNextPrey(i.Unit).Name}.</b>", priority: 9, conditional: s=> ScatPossible(s) && (CanAddressPlayer(s) || PotentialNextPrey(s.Unit).Name != "You, the player" )),
            new EventString((i) => $"<b>{i.Unit.Name}</b> is proud of the steaming mound of shit he turned <b>{i.Target.Name}</b> into, and desires to add <b>{PotentialNextPrey(i.Unit).Name}</b> on top while it's warm.", priority: 9, conditional: s=> ScatPossible(s) && (CanAddressPlayer(s) || PotentialNextPrey(s.Unit).Name != "You, the player" )),
            new EventString((i) => $"<b>{i.Target.Name}</b> is nothing but {GetRandomStringFrom("a gross brown mass", "stinking turds", "fresh, warm shit", "disgusting waste")} that's now leaving <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> rectum", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> guts are done with <b>{i.Target.Name}</b>, leaving {GPPHim(i.Target)} to be disposed of as nothing but {GetRandomStringFrom("a gross brown pile", "stinking turds", "fresh, warm shit", "disgusting waste", "a creamy dump")}.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"With a squint and a push from <b>{i.Unit.Name}</b>, <b>{i.Target.Name}</b> manages to achieve {GPPHis(i.Target)} final form as {InfoPanel.RaceSingular(i.Unit)} droppings.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> grunts, and the filthy remains of {GPPHis(i.Unit)} now used-up prey splat onto the ground behind {GPPHim(i.Unit)}. <b>{i.Target.Name}</b> is completely unrecognizable.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"All that's left of <b>{i.Target.Name}</b> is a {InfoPanel.RaceSingular(i.Target)}-sized load of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> feces. Good fight.", priority: 9, conditional: s => ScatEnabled()),
            new EventString((i) => $"<b>{i.Unit.Name}</b>: <i>~PFRRRRRT~</i>. <b>{i.Target.Name}</b>: <i>~Splat~</i>", priority: 9, conditional: s => ScatPossible(s) && Farts()),
            new EventString((i) => $"A cacophonous fart out of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> hind side heralds <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> smelly return to the battlefield...", priority: 9, conditional: s => ScatPossible(s) && Farts()),
            new EventString((i) => $"<b>{i.Unit.Name}</b> relieves {GPPHimself(i.Unit)}...\nThe obscene release of gas will have to suffice as <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> eulogy.", priority: 9, conditional: s => ScatPossible(s) && Farts()),
            new EventString((i) => $"With the amount of shit piling up under <b>{i.Unit.Name}</b>' ass, one could think <b>{i.Target.Name}</b> was mostly waste to begin with.", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> exerts {GPPHimself(i.Unit)}, letting loose one juicy burst of gas, before having <b>{i.Target.Name}</b> emerge and drop out of that same hole to form a pile of {GetRandomStringFrom("nasty", "creamy", "warm", "greasy")} {GetRandomStringFrom("logs", "turds", "brown lumps")}", priority: 9, conditional: s => ScatPossible(s) && Farts()),
            new EventString((i) => $"<b>{i.Target.Name}</b> has been reduced to {GetRandomStringFrom("shit", "waste", "a dump", "scat")} by <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> digestive system.", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"Just like that, <b>{i.Target.Name}</b> has been casually shat onto the ground as mere {InfoPanel.RaceSingular(i.Unit)} waste, and <b>{i.Unit.Name}</b> certainly won't remember what {GPPHe(i.Unit)} ate.", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> raises {GPPHis(i.Unit)} tail and closes {GPPHis(i.Unit)} eyes as <b>{i.Target.Name}</b> is finally set free - in a form that not even a mother could recognize.", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> shits a load of <b>{i.Target.Name}</b> onto the floor", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"<b>{i.Unit.Name}</b> thinks <b>{i.Target.Name}</b> looks better as this neat arrangement of {GetRaceDescSingl(i.Unit)} turds", priority: 9, conditional: s => ScatPossible(s)),
            new EventString((i) => $"After a bit of a messy dump, <b>{i.Unit.Name}</b> wonders which turd could be that {InfoPanel.RaceSingular(i.Target)} leader, but can't quite tell.", priority: 9, conditional: s => ScatPossible(s) && TargetLeader(s)),
            new EventString((i) => $"The friction from <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> remains sliding past {GPPHis(i.Unit)} anal sphincter sends arousing tingles through <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> body.", priority: 9, conditional: s => ScatPossible(s) && Lewd()),

            //Fart disposal
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> sphincter blows <b>{i.Target.Name}</b> a smelly requiem. Looks like {GPPHis(i.Unit)} innards are done with their prey.", priority: 9, conditional: s => Farts() && InStomach(s)),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> rump is eager to announce that <b>{i.Target.Name}</b> is completely used up, and has a loud, satisfying fart to show for it.", priority: 9, conditional: s => Farts() && InStomach(s)),
            new EventString((i) => $"R.I.P <b>{i.Target.Name}</b>, Turn {Math.Max(State.World.Turn - (int)(Math.Pow(i.Target.Level,1.5)/2), 1)} - Turn {State.World.Turn}. Once Intrepid member of the {InfoPanel.RaceSingular(State.World?.GetEmpireOfSide(i.Target.Side))} Empire military forces, now an assfull of {InfoPanel.RaceSingular(i.Unit)} farts" + (ScatEnabled() ? $" and shit" : $""), // Arbitrary formula arbitrarily seeking to correlate level with time alive in an arbitrary manner
            priority: 9, conditional: s => InStomach(s) && Farts() && TargetHumanoid(s) && !State.GameManager.PureTactical),
            new EventString((i) => $"<b>{i.Unit.Name}</b> passes gas – <b>{i.Target.Name}</b> passes on to the next life.", priority: 9, conditional: s => Farts() && !ScatPossible(s) && InStomach(s)),
            new EventString((i) => $"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> ass vibrates loudly as it releases <b>{i.Target.Name}</b> as a gust of stink.", priority: 9, conditional: s => Farts() && !ScatPossible(s) && InStomach(s)),
            new EventString((i) => $"<b>{i.Target.Name}</b> has become nothing but an obscene melody under <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> tail", priority: 9, conditional: s => Farts() && !ScatPossible(s) && ActorTail(s) && InStomach(s)),
            ...
            new EventString((i) => $"With nothing to absorb anymore, <b>{i.Unit.Name}</b> cums out the puddle of <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> remains.", priority: 9, conditional: s => InBalls(s) && !Config.CondomsForCV),
            ...
            actorRace: Race.Dragon, targetRace: Race.Kobolds, priority: 9, conditional: s => InBalls(s) && Friendly(s) && !Config.CondomsForCV),

            new EventString((i) => $"\"Attention {GetRaceDescSingl(i.Target)}, this is what has become of your 'mighty' leader,\" <b>{i.Unit.Name}</b> taunts as {GPPHe(i.Unit)} jerk{SIfSingular(i.Unit)} {GPPHimself(i.Unit)} to orgasm, splattering the remains of <b>{i.Target.Name}</b> {(Config.CondomsForCV ? $"into a condom, promptly tying it and tossing it aside." : "all over the ground in an impressive puddle.")}",
            priority: 9, conditional: s => ActorLeader(s) && InBalls(s) && TargetLeader(s)),

            new EventString((i) => $"As what little is left of <b>{i.Target.Name}</b> is pumped out <b>{i.Unit.Name}</b>'s of {PreyLocStrings.ToCockSyn()}, {GetRandomStringFrom($"{GPPHis(i.Target)}", $"the {GetRaceDescSingl(i.Target)}'s")} remains are captured within the semi-translucent rubber of a condom.", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"Feeling that <b>{i.Target.Name}</b>'s digestion is complete, <b>{i.Unit.Name}</b> slips a condom over {GPPHis(i.Unit)} {PreyLocStrings.ToCockSyn()}, into which {GetRandomStringFrom($"{GPPHe(i.Unit)}", $"the {GetRaceDescSingl(i.Target)}")} promptly pumps <b>{i.Target.Name}</b>.", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"After emptying {GPPHis(i.Unit)} {PreyLocStrings.ToBallSynPlural()} into a condom, <b>{i.Unit.Name}</b> observes {GPPHis(i.Unit)} handiwork. \"Between a {GetRaceDescSingl(i.Target)} and a condom filled with {GetRaceDescSingl(i.Unit)} {PreyLocStrings.ToFluid(PreyLocation.balls)}, I gotta say, you look better in your new form.\"", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"After emptying {GPPHis(i.Unit)} {PreyLocStrings.ToBallSynPlural()} into a condom, <b>{i.Unit.Name}</b> observes {GPPHis(i.Unit)} handiwork. \"And with that, your makeover into my {PreyLocStrings.ToFluid(PreyLocation.balls)} is complete.\"", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"As <b>{i.Unit.Name}</b> feels {GPPHis(i.Unit)} balls contract, {GPPHe(i.Unit)} barely {HasHave(i.Unit)} time to get the condom over {GPPHis(i.Unit)} {PreyLocStrings.ToCockSyn()} before <b>{i.Target.Name}</b> explodes out into the contraceptive.", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"With no fanfare, <b>{i.Unit.Name}</b> fills a condom with what little remains of <b>{i.Target.Name}</b>.{GetRandomStringFrom("", $"As {GetRandomStringFrom($"{GPPHe(i.Unit)} tie{SIfSingular(i.Unit)}", $"the {GetRaceDescSingl(i.Unit)} ties")} off the condom, {GPPHe(i.Unit)} silently whisper{SIfSingular(i.Unit)}, \"You'll make a good trophy.\"", $"As {GetRandomStringFrom($"{GPPHe(i.Unit)} tie{SIfSingular(i.Unit)}", $"the {GetRaceDescSingl(i.Unit)} ties")} off the condom, {GPPHe(i.Unit)} silently whisper{SIfSingular(i.Unit)}, \"Please don't pop on me. I do not need the mess from that.\"", $"As {GetRandomStringFrom($"{GPPHe(i.Unit)} tie{SIfSingular(i.Unit)}", $"the {GetRaceDescSingl(i.Unit)} ties")} off the condom, {GPPHe(i.Unit)} silently whisper{SIfSingular(i.Unit)}, \"Please don't pop on me. I want you as a trophy.\"")}", priority: 9, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),

            new EventString((i) => $"After emptying {GPPHis(i.Unit)} {PreyLocStrings.ToBallSynPlural()} into a condom, <b>{i.Unit.Name}</b> observes {GPPHis(i.Unit)} handiwork. \"Between a Slime and a condom filled with {GetRaceDescSingl(i.Unit)} {PreyLocStrings.ToFluid(PreyLocation.balls)}, I gotta say, I can't really tell a difference.\"", priority: 9, targetRace: Race.Slimes, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"After emptying a former Slime from {GPPHis(i.Unit)} {PreyLocStrings.ToBallSynPlural()} into a condom, <b>{i.Unit.Name}</b> observes {GPPHis(i.Unit)} handiwork. \"Huh. If I didn't know better, I'd almost wonder if you actually got digested at all.\"", priority: 9,targetRace: Race.Slimes,  conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            new EventString((i) => $"With no fanfare, <b>{i.Unit.Name}</b> fills a condom with what little remains of <b>{i.Target.Name}</b>. As {GetRandomStringFrom($"{GPPHe(i.Unit)} tie{SIfSingular(i.Unit)}", $"the {GetRaceDescSingl(i.Unit)} ties")} off the condom, {GPPHe(i.Unit)} silently whisper{SIfSingular(i.Unit)}, \"{GetRandomStringFrom($"You don't look to have changed one bit, you stupid Slime.", $"Would you look at that? You're still a Slime after all.")}\"", priority: 9, targetRace: Race.Slimes, conditional: s => ActorHumanoid(s) && InBalls(s) && Config.CondomsForCV),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b>'s pouch has shrunk back down to normal size and shape, with no evidence that a {GetRaceDescSingl(i.Target)} had ever been inside.", actorRace: Race.Kangaroos, priority: 10),
            new EventString((i) => $"With <b>{i.Target.Name}</b> fully absorbed, <b>{i.Unit.Name}</b>'s pouch unseals, revealing not one hint as to what happened to its previous {GetRandomStringFrom("occupant", "tenant")}.", actorRace: Race.Kangaroos, priority: 10),
            ...
            actorRace: Race.Abakhanskya, priority: 9),
            new EventString((i) => $"A wet belch stains the air. <b>{i.Target.Name}</b> no longer bloats <b>{i.Unit.Name}'s</b> titanic belly.",
            actorRace: Race.Abakhanskya, priority: 9),
            //Aba Lewd
            new EventString((i) => $"Abakhanskya grins and rumbles with content as her tits drizzle their rich product. <b>{i.Target.Name}</b>'s harvesting was the catalyst for this bounty.",
            ...
            new EventString((i) => $"As <b>{i.Unit.Name}</b> hurries away, the last drops of \"<b>{i.Target.Name}</b>\" drip from {GPPHis(i.Unit)} {GetRandomStringFrom("vagina", "slit", "muff")} without the predator's noticing.",
            priority: 10),
            new EventString((i) => $"The bouncing up and down from <b>{i.Unit.Name}</b>'s attempt to escape the battlefield forces the last of <b>{i.Target.Name}</b> out of <b>{i.Unit.Name}</b>'s {PreyLocStrings.ToSyn(PreyLocation.breasts)} onto the ground as a pathetic puddle of milk.",
            priority: 10),
            new EventString((i) => $"With a jolt, <b>{i.Target.Name}</b> erupts from <b>{i.Unit.Name}</b>'s {PreyLocStrings.ToCockSyn()} as a gush of {PreyLocStrings.ToFluid(PreyLocation.balls)}. <b>{i.Unit.Name}</b> pauses {GPPHis(i.Unit)} fleeing for a moment and mutters \"Didn't even get to enjoy you...\" before continuing {GPPHis(i.Unit)} flight.",
            priority: 10),
            new EventString((i) => $"Feeling a specific urge, <b>{i.Unit.Name}</b> halts {GPPHis(i.Unit)} running to place a condom over {GPPHis(i.Unit)} {PreyLocStrings.ToCockSyn()}, rubs the last of <b>{i.Target.Name}</b> out into it, and then continues running.",
            priority: 10),
            */
        ...
        TransferMessages = new List<EventString>()
        {
            ...
            priority: 11, conditional: s => InStomach(s) && Lewd()),
            ...
            priority: 11, conditional: s => !InStomach(s) && Lewd() && !s.Prey.IsDead ),
            ...
            new EventString((i) =>$"As <b>{i.Unit.Name}</b>'s [WFB] shrinks, <b>{i.Prey.Name}</b>{(i.Prey.IsDead ? "'s liquefied form fills" : " hits")} the thin rubber barrier between {GetRandomStringFrom($"<b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b>", $"the {GetRaceDescSingl(i.Unit)}'s")} {PreyLocStrings.ToCockSyn()} and <b>{i.Target.Name}</b>'s {PreyLocStrings.ToSyn(PreyLocation.womb)} until, with a *pop* sound, bursting through.{GetRandomStringFrom("", $" <b>{i.Unit.Name}</b> looks sheepishly at <b>{i.Target.Name}</b>. \"{GetRandomStringFrom("Oops?", "Oops.", "Sorry?", "Sorry.")}\"")}",
            priority: 11, conditional: a => Config.CondomsForCV && Lewd()),
            new EventString((i) =>$"As <b>{i.Unit.Name}</b> goes to have sex with <b>{i.Target.Name}</b>, {GPPHe(i.Unit)} pull{SIfSingular(i.Unit)} out a condom, before being told to put it away. Without that barrier of protection, the {PreyLocStrings.ToFluid(PreyLocation.balls)} that was once <b>{i.Prey.Name}</b> spills into <b>{i.Target.Name}</b>'s {PreyLocStrings.ToSyn(PreyLocation.womb)} unimpeded.",
            priority: 11, conditional: a => Config.CondomsForCV && Lewd() && a.Prey.IsDead),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> slips on a condom and goes to have sex with <b>{i.Target.Name}</b>. After they're done, <b>{i.Unit.Name}</b> observes that the former <b>{i.Prey.Name}</b> is leaking from 7 small holes in the condom. Knowing what happened, <b>{i.Unit.Name}</b> shouts \"<b>{GetRandomAlly(i.Unit, i.Target, i.Prey)}</b>! Not funny!\"",
            priority: 11, conditional: a => Config.CondomsForCV && Lewd() && a.Prey.IsDead && GetRandomAlly(a.Unit, a.Target, a.Prey) != null),
            new EventString((i) =>$"Before having sex, <b>{i.Unit.Name}</b> looks for {GPPHis(i.Unit)} condom, but can't find it. \"I think I left it with <b>{GetRandomAlly(i.Unit, i.Target, i.Prey)}</b>. Want me to go get it?\" <b>{i.Target.Name}</b> responds \"Nah, I saw your face when that {GetRaceDescSingl(i.Prey)} went in your balls, I want to feel that myself.\" With that, the two {GetRandomStringFrom("have sex", "fuck")}, right there in the middle of the battlefield.",
            priority: 11, conditional: a => Config.CondomsForCV && Lewd() && GetRandomAlly(a.Unit, a.Target, a.Prey) != null),
        };
        ...
        GreatEscapeKeepMessages = new List<EventString>()
        {
          ...
          targetRace: Race.Erin, priority: 26, conditional: s => Lewd() && HasGreatEscape(s) && InStomach(s)),
          ...
          targetRace: Race.Erin, priority: 26, conditional: s => Cursed(s) && Lewd()),
          ...
          targetRace: Race.Erin, priority: 26, conditional: s => Cursed(s) && Lewd() && InStomach(s)),
          ...
          targetRace: Race.Erin, priority: 26, conditional: s => HardVore() && HasGreatEscape(s) && InStomach(s)),
